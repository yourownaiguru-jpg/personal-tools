...
export function template(state: AppState): string {
  ...
  <div style="display:grid;grid-template-columns:minmax(0,1fr);gap:22px">
    <div class="field">
      ...
      <input id="name-input" class="input" style="font-size:24px;min-height:56px;padding:8px 14px" value="${escapeHtml(state.text)}" placeholder="e.g. Kaveri · காவேரி · ಕಾವೇரி" autocomplete="off" spellcheck="false">
    </div>
    <div class="field">
      <label>Language</label>
      <div style="display:flex;flex-wrap:wrap;gap:8px">${langButtonsHtml(state)}</div>
    </div>
  </div>
  ...
  <div style="margin-top:40px;display:flex;justify-content:space-between;align-items:flex-end;flex-wrap:wrap;gap:12px">
    <div style="display:flex;flex-direction:column;gap:6px">
      <h2 style="font-size:34px;font-weight:400;margin:0">${escapeHtml(era.name)}</h2>
      <span style="font-size:13px;color:var(--color-neutral-600);font-variant-numeric:tabular-nums">${escapeHtml(era.years)} · ${escapeHtml(lang.name)} · ${escapeHtml(SCRIPT_LABEL[era.script])}</span>
    </div>
    <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
      <div class="seg">
        ${(['stone', 'copper', 'palm'] as const)
      ...
      <button type="button" class="btn btn-secondary" data-action="share">${escapeHtml(state.shareLabel)}</button>
    </div>
  </div>
  ...
  <div class="plate" style="filter:none;margin-top:16px;border-radius:2px;background:${style.wrapBg}">
    <div style="position:relative;min-height:320px;display:grid;place-items:center;padding:56px 32px;text-align:center;background:${style.bg};box-shadow:inset 0 0 60px rgba(0,0,0,.4)">
      ...
      <div style="font-family:${escapeHtml(`"${S.font}"`)};font-size:96px;line-height:1.45;color:${style.ink};opacity:${style.inkOpacity};mix-blend-mode:${inkBlend};text-shadow:${style.shadow};overflow-wrap:anywhere;max-width:100%;position:relative">${out.trim() ? escapeHtml(out) : '·'}</div>
    </div>
  </div>
  <p style="font-size:12px;color:var(--color-neutral-600);margin-top:10px;text-align:justify">${escapeHtml(era.approx)}</p>
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:32px">
    <div>
      <h6 style="color:var(--color-accent);margin-bottom:8px">${escapeHtml(era.years)}</h6>
      <h3 style="font-weight:400;font-size:30px">${escapeHtml(era.name)}</h3>
      <p style="text-align:justify;font-size:15px;line-height:1.65;margin:0">${escapeHtml(era.note)}</p>
    </div>
    <div>
